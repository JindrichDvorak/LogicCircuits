...
import { stateAdd, logicNeg } from "../../../logic";
import imgSVG from "/NOR.svg?raw";
...
export class NORgate extends Component {
    ...
    createComponent(nodeManager) {
        ...
        this.aNode = nodeManager.createOutputNode(0, 11.5, 0, 0, true);
        this.nodes.push(this.aNode);
        this.aState = this.aNode.logicState;

        this.bNode = nodeManager.createOutputNode(0, this.size.height - 17, 0, 0, true);
    ...
    componentLogic(aState, bState) {
        return logicNeg(stateAdd(aState, bState));
    }
}